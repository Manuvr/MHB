function Exec(models, argParser) {
  ...
  var propsInArg = measures.length;
	...
	this.IMU_MAP_STATE = function (jsonBuff) {
		...
		gmUpdated = lm.reduce(function(obj, val, i, arr) {
		  ...
		  //console.log('argArr: ', argArr[i], ' bone, prop: ', boneName, propName);
  ...
  this.IMU_TAP = function (jsonBuff) {
    console.log(jsonBuff);
  }

  this.IMU_DOUBLE_TAP = function (jsonBuff) {
    console.log(jsonBuff);
  }
	...
	this.messageRef =  {
		...
		1546: 	{ callback: this.IMU_TAP, 		  reply: false, refresh: true , argParse: false, type: 'IMU_TAP'},
		1547: 	{ callback: this.IMU_DOUBLE_TAP,  reply: false, refresh: true , argParse: false, type: 'IMU_DOUBLE_TAP'},

		// These are HARD CODED
		0:		{ callback: undefined           , reply: false, refresh: false, argParse: false, type: 'LOG_TO_CO'},
		...
		// test case for argument builder
