function Exec(models, argParser) {
  ...
  // Map Glove Object for every frame read
    ...
    gmUpdated = argArr.reduce(function(obj, val, i, arr) {
      // TODO: Check against legend?
      // Figure out which IMU we are on
      // i.e. CAPRALS, METACARPALS, PP_1, IP_1...
      currentIMU = Math.floor(i / propsInArg);

      // Figure out which property in the IMU we are setting
      // i.e. quat, accel, mag, gyro, temp
      currentIMUProperty = i % propsInArg;

      // Set the property for the IMU
      obj[currentIMU][currentIMUProperty] = val;
    ...
    /*
		if (Object.keys(argObj).length === 72) {
			var argObjSub = Object.keys(argObj[argObj[0]]);
			for (var b = 0; b < Object.keys(gm).length; b++) {
				var imuBase = Object.keys(gm)[b];
				console.log(Object.keys(gm)[b]);
				var imuBaseProperties = Object.keys(gm[imuBase]);
				var tick = 1;
				for(var r = 0; r < 4; r++){
					for(var c = 0; c < 4; c++){
						gm[imuBase][imuBaseProperties[0]] =  argObj[r + (b * 4)]
					}
					for(var t = 0; t < 3; t++){
						gm[imuBase][imuBaseProperties[tick]] = argObj[r + (b * 4)][argObjSub[t]];
						tick++;
					}
				}
				console.log(argObj[3 + (b * 4)]);
				gm[imuBase][imuBaseProperties[9]] = argObj[3  + (b * 4)];
			}
		} else {
