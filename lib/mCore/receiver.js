...
var syncPacket = new Buffer([0x00, 0x00, 0x55], 'hex');
...
function Receiver() {
  ...
  this.parser = Dissolve().loop(function(end) {
    ...
    if (!that.selfSync) {
      console.log("RECEIVER is NOT self-syncd");
      this.uint8('wait')
        .tap(function() {
          if(this.vars.wait === 0x04){
            this.buffer('check', 3)
              .tap(function() {
      ...
      this.uint32le('check')
        .tap(function() {
          console.log("PARSING AND SYNCED NAO");
          
          that.timer = setTimeout(function() {
            console.log("PARSER TIMED OUT ON READ");
            end()
          }, that.parseTimeOut);
