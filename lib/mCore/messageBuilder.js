...
var packOwnLegendMessages = function(msg_defs) {
  ...
  for (var msg_def in msg_defs) {
    if (msg_defs.hasOwnProperty(msg_def)) {
      ...
      return_value.writeUInt16LE(parseInt(msg_def), offset);
      return_value.writeUInt16LE(msg_def.flag, offset + 2);
      return_value.writeUInt8(msg_def.minimumArgs, offset + 3);
      offset += 5;
      ...
      return_value.write(msg_def.argForm, offset, 'ascii'); // +(some more) for the string to represent the argument form.
      offset += msg_def.name.length;
      return_value[offset++] = 0;
    }
  }
