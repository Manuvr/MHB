...
function getPotentialArgFormsByCardinality(messageDef, card) {
  if(card >= messageDef.minimumArgs) {
    return messageDef.argForm
  }
  return []
  // var return_value = [];
...
var builder = function(messageDefs, types, jsonBuff) {
  ...
  if (jsonBuff.args && jsonBuff.args.length > 0) {
    //var arg_form = getPotentialArgFormsByCardinality(messageDefs[jsonBuff.messageId], jsonBuff.args.length);
    ...
    //   return false;
    // }
    var arg_form = messageDefs[jsonBuff.messageId].argForm;
    var parseType;
    var temp_buffer;

    console.log('builder argform:   ' + JSON.stringify(arg_form));
    ...
    console.log('Flattened args: ' + JSON.stringify(flattened_args));
  }
