...
function stateMachina(parent){
    ...
    states: {
      ...
      noSync: {
        ...
        receivedReply: function() {
          console.log("ZOMG RECEIVED-REPLY TOO EARLY!");
        }
      },
      waitingForKA: {
        _onEnter: function() {
    ...
    initialize: function(){
      ...
      this.clearAck = function(){
        ...
        this.ackPool.forEach( function(cur, idx, arr){
          clearTimeout(cur.ackTimeout);
          delete cur.ackTimeout;
          this.outBoundQueue.unshift(arr.shift())
        }.bind(this))
      }.bind(this);
    },
    initialState: "normal",
      ...
      processQueue: {
        ...
        _onEnter: function(){
          while(this.outBoundQueue.length > 0){
            this.emit('sendToTransport', this.outBoundQueue[0]);
            if(this.requiresAck(this.outBoundQueue[0])){
              this.ackPool.push(this.outBoundQueue.shift());
              //blocked
              break;
            } else {
          ...
          this.transition(this.outBoundQueue.length > 0 ? 'outputBlocked' : 'normal');
        }
      }
    }
  })
 ...
 // |_|  |_/_/ \_\___/ |_| |___|_|_\
  ...
  // TODO: Set up proper logging of the state machine with an external logger.
      ...
      case 'SELF_DESCRIBE':
        ...
        switch(msgObj.args.length){
          case 0:
            ...
            //   }
            // }
            console.log("I'M JUST ACKING AN EMPTY SELF_DESCRIBE: " + JSON.stringify(msgObj, null, 2))
            buildEmit( ackMessage(msgObj.uniqueId ) )
            break;
          case 8:
          ...
          default:
            console.log("Sending a SELF DESCRIBE ack: " + JSON.stringify(msgObj))
            buildEmit( ackMessage(msgObj.uniqueId) )
            parseEmit( outObj )
        }
        break;
