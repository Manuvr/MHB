...
function stateMachina(parent){
  ...
  var masterState = new Machina.Fsm({
    ...
    states: {
      disconnected:{
        syncPacket: function() {
          this.deferUntilTransition('syncPacket')
  ...
  syncState.on('sendSyncKA', function(uniqueId){
    console.log("SENDING KA " + uniqueId)
    buildEmit(kaMessage(uniqueId));
  });

  syncState.on('sendSync', function(){
  ...
  runningSession.on('sendToTransport', function(msgObj){
    buildEmit(msgObj);
  });
  ...
  this.clientIn = function(msgObj){
    var test = ["messageId", "messageDef", "uniqueId", "flag", "args"]
    if (test.every(function(x) { return x in msgObj })) {
      masterState.handle("msg_" + messageDef, msgObj);
    } else {
