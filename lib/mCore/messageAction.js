...
function parseAct(jsonBuff) {
  ...
  var ackMessage = function() {
    that.emit('doneBuilding', 'data',
      that.buildBuffer(that.mLegend, that.types,
        {
          "messageId":  1,
          "messageDef": 'REPLY',
  ...
  switch (jsonBuff.messageName) {
    case 'LEGEND_MESSAGES':
      ...
      _merge(this.mLegend, jsonBuff.message);
      this.emit('doneParsing', 'data', jsonBuff)
      ackMessage();
      break;
    case 'KA':
      // When we get a KA, we should ACK it immediately.
      this.emit('input', {target: ['syncd'], data: true})
      ackMessage();
      break;
    case 'REPLY':
