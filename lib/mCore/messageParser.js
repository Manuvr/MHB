...
function getPotentialArgForms(messageDef, len, types) {
  ...
  console.log('Potential argForms: '+return_value);
  return return_value;
}
  ...
  if (jsonBuff.raw && jsonBuff.raw.length !== 0) {
    ...
    if (handlers.length > 0) {
      for (var key in handlers) {
        ...
        while ((i < buffLen) && (argNum < handlers[key].length)) {
        ...
        if ((outObj.length == handlers[key].length) && (i == buffLen)) {
...
MessageParser.prototype.parse = function(jsonBuff) {
  ...
  jsonBuff.args = jsonBuff.messageCode === 11 ?
