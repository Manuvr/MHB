  ...
  while (i < jsonBuff.raw.length) {
    ...
    while (current_type_code !== 0) {
      if (types.hasOwnProperty(current_type_code)) {
        ...
        tempObj[msg_code].argForm.push(types[current_type_code]);
...
function typeParse(jsonBuff, messageDefs, types) {
  ...
  if (jsonBuff.raw && jsonBuff.raw.length !== 0) {
    ...
    //  jsonBuff.raw.length,
    //  types);
    var handler = messageDef.argForm;
    console.log('typeParse argform:   ' + JSON.stringify(handler));
    ...
    while ((i < buffLen) && (argNum < handler.length)) {
      var currentTypeCode = handler[argNum];
    ...
    if ((outObj.length == handler.length) && (i == buffLen)) {
