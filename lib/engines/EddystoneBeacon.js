...
function mEngine(parent) {
  ...
  that.nonce        = (Math.random() * (Math.pow(2, 32) - 1)) & 0x00FFFFFF;
  ...
  esBeacon.setBatteryVoltage(batteryVoltage);
  esBeacon.setTemperature(temperature);
  esBeacon.advertiseUid(that.namespace, that.nonce, that.options);
  esBeacon.advertiseUrl(that.url, that.options);
