    ...
    if (typeof xport === 'string') {
      _forOwn(that.interface_spec.adjuncts,
        function(val, key, obj){
          console.log('xport selection loop: '+key+'  '+util.inspect(val));
      ...
      if (xport.interface_spec.schema.type !== 'mTransport') {
    ...
    schema: {
      ...
      inputs: {
        ...
        'log': {
          ...
          args: [ { label: '?', type: 'boolean' } ],
          func: function(me, data) {
            that.log('client', data.body, (data.verbosity) ? data.verbosity : 7);
          },
          hidden: true
        },
        'newSession': {
          ...
          func: function(me, data) {
            that.buildNewSession(data.shift(), data.shift(),
              function(err) {
                if (err) {
        ...
        'assignEngine': {
          ...
          func: function(me, data) {
            ...
            if (me.interface_spec.adjuncts[data[1]]) {
              ...
              sendToAdjuct(data[0], ['assign'], xprtObj);
            }
          }
        },
        ...
        'test' : {
          ...
          func: function(me, data){
            ...
            me.send({target:["log"], data: "I'm data!"});
          }
        }
      },
      outputs: {
    ...
    taps: {
      ...
      "mTransport": {
        ...
        "connected": function(me, msg, adjunctID){
          // If this is coming from a transport with no session, we instance a session and 
          //   assign the xport.
