/*
* This file is top-level interface to MHB. All usage of MHB can be sensibly
*   directed from this class. It contains the sessions, the means of loading
*   transports and engines, and the machinary to associate them.
*/
...
function mHub(cli_config) {
      ...
      if (err) {
        ...
        for (var i = 0; i < items.length; i++) {
          if ('.js' == items[i].substr(items[i].length - 3).toLowerCase()) {
            ...
            var temp_req = require(path+items[i]);
            var tfact_name = items[i].substr(0, items[i].length - 3);
  ...
  */
    ...
    schema: {
      inputs: {
        ...
        'newSession': {
          ...
          args: [{ label: 'Transport Name', type: 'string', def: 'tcp' },
        ...
        'assignEngine': {
          label: "Assign Engine",
          args: [{ label: 'Session Name', type: 'string', def: 'NewUnnumbered' },
                 { label: 'Engine Name', type: 'string', def: 'DeaconMgmt.js' }],
          func: function(me, data) {
      ...
      'types': {
        ...
        "mTransportFactory": {
          ...
          "connected": function(me, msg, adjunctID){
            //me.buildNewSession(msg.data, 'New' + adjunctID + Math.random().toString().slice(2, 6));
            me.buildNewSession(msg.data, adjunctID + me.inc);
            me.log(msg.target.toString(), 'Got a connected broadcast from '+adjunctID, 7);
            return false;
          },
          "scanResult": function(me, msg, adjunctID){
            me.log(msg.target.toString(), 'Got a scanResult from '+adjunctID, 7);
            return false;
          },
        },
        "mSession": {
          "SELF_DESCRIBE": function(me, msg, adjunctID){
            ...
            for (var eng in me.engines) {
              ...
              if (name === engineConfig.counter_identity) {
  ...
  // DELETE ME LATER.  ONLY TEMPORARY FOR REGISTRY PURPOSES, UNTIL
  // ROUTER IS MORE FLESHED OUT
  var genEngine = "NewUnnumbered"
  var testReq   = require('../registry.js');
  this.mH.addAdjunct('Registry', new testReq());
  this.interface_spec.taps.names["Registry"] = {
    "thingId": function(me, msg, adjunctID){
      console.log("Sending new Thing ID...")
      me.mH.sendToAdjunct(msg.data[0], {target: ["bless", "metaSchema"], data: msg.data[1]})
      return true;
    },
    "claimed": function(me, msg, adjunctID){
      console.log("Sending claimed...")
      me.mH.sendToAdjunct(msg.data[0], {target: ["takeOwnership", "metaSchema"], data: msg.data[1]})
      return true;
    }
  }
