      ...
      if (err) {
        ...
        for (var i = 0; i < items.length; i++) {
          if ('.js' == items[i].substr(items[i].length - 3).toLowerCase()) {
            ...
            that.interface_spec.adjuncts.transportFactories[items[i]] = temp_req.IFSpec;
            loaded++;
            that.toClient({    
              target: ['transportFactories', items[i], '_adjunctDef'],
              data:   that.interface_spec.adjuncts.transportFactories[items[i]]
            });
          }
          else {
