...
function mHub(cli_config) {
  ...
  */
  this.log = function(origin, body, verbosity) {
    ...
    var c_timestamp = new Date().getTime()/1000;
    var _ver = verbosity ? verbosity : 7;
    if (current_log_file) {
      // Write to the log file if we have one open.
      fs.writeSync(current_log_file, _ver + '\t' + c_timestamp + '\t' +origin+ "\t" + body + '\n');
    }
  ...
  */
  var loadEngines = function(path) {
    ...
    fs.readdir(path, function(err, items) {
      var loaded = 0;
      if (err) {
        that.log('mHub', 'Failed to load engines because ' + err);
      }
      else {
        ...
        that.log('mHub', 'Loaded ' + loaded + ' engines.');
      }
    });
  };
  ...
  */
  var loadTransports = function(path) {
    ...
    fs.readdir(path, function(err, items) {
      var loaded = 0;
      if (err) {
        that.log('mHub', 'Failed to load transports because ' + err);
      }
      else {
        for (var i = 0; i < items.length; i++) {
          if ('.js' == items[i].substr(items[i].length - 3).toLowerCase()) {
        ...
        that.log('mHub', 'Loaded ' + loaded + ' transports.');
      }
    });
  };
  ...
  this.buildNewSession = function(xport, name, callback) {
    ...
    if (typeof xport === 'string') {
      if (that.interface_spec.adjuncts.hasOwnProperty(xport)) {
        if (that.mH.getAdjunctType(xport) === 'mTransport') {
          var named_adj = that.interface_spec.adjuncts[xport];
          xport_worker = named_adj.instance.getWorker();
        }
        else {
          that.log('mHub', "'"+xport+"' names a  "+that.mH.getAdjunctType(named_adj)+", not an mTransport.", 3);
        }
      }
      else {
        that.log('mHub', "There is no mTransport named '"+xport+"'.", 3);
      }
      if (!xport_worker) {
