      ...
      if (err) {
        ...
        for (var i = 0; i < items.length; i++) {
          if ('.js' == items[i].substr(items[i].length - 3).toLowerCase()) {
            ...
            that.transports[items[i]] = require(path+items[i]).init();
            that.interface_spec.transportFactories[items[i]] = that.transports[items[i]].IFSpec();
            loaded++;
          }
          else {
        ...
        if (loaded > 0) {
          that.toClient(
            {    
              ...
              data:   Object.keys(that.interface_spec.transportFactories)
            }
          );
        }
