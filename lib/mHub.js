      ...
      if (err) {
        ...
        for (var i = 0; i < items.length; i++) {
          if ('.js' == items[i].substr(items[i].length - 3).toLowerCase()) {
            ...
            that.transports[tfact_name] = temp_req.init();
            ...
            that.transports[tfact_name].on('output',
              function(message) {
                switch (message.target[0]) {
                  ...
                  default:
                    that.toClient({
                      target: message.target.unshift(['transportFactories', tfact_name]),
                      data:   message.data
                    });
                    break;
                }
  ...
  this.buildNewSession = function(xport, name, callback) {
    ...
    if (typeof xport === 'string') {
      if (that.transports.hasOwnProperty(xport)) {
        xport_worker = that.transports[xport].getWorker();
      }
      else {
    ...
    if (name != undefined && name !== '') {
      if (!that.sessions.hasOwnProperty(name)) {
        ...
        that.sessions[name].on('toClient', function(message) {
            ...
            switch (message.target[message.target.length-1]) {
              case '_adjunctDef':
                ...
                that.interface_spec.adjuncts.sessions[name] = data;
                break;
              case 'log':
            ...
            that.toClient({
              target: message.target.unshift(['sessions', name]),
              data:   data
            });
          }
        );
  ...
  */
  this.toHub = function(message) {
    ...
    if (message.data && message.data.length) {
      ...
      var tempArray = [];
      if(message.target.length > 2) {
      //  _forEach(message.target.slice(0, message.target.length - 2), function(n){
      //    tempArray.push(n, "adjuncts");
      //  })
        tempArray = new Array(message.target);
        tempArray.pop();
        tempArray.unshift('adjuncts');
      }
      //tempArray.push(message.target[message.target.length - 2])
      //tempArray.unshift('adjuncts');
      
      // TODO, re-code as map
      for (var i = 0; i < message.data.length; i++) {
        var datum = message.data[i];
        console.log(util.inspect(that.interface_spec) + "\n\nTarget verify: "+util.inspect(_get(that.interface_spec, tempArray))+'\ntempArray:  '+util.inspect(tempArray));
        switch(_get(this.interface_spec, tempArray.concat("schema", "inputs", method))[i].type) {
          case 'boolean':
            message.data[i] = Boolean(datam);
            break;
          case 'float':
            if (typeof datum == 'number') {
              message.data[i] = parseFloat(datam);
            }
            else {
    ...
    if (message.target.length > 1) {
      ...
      if (that.interface_spec.hasOwnProperty(adj_target)) {
        that.interface_spec.adjuncts[adj_target].input(message);
      }
    }
    else {
      ...
      switch (method) {
        case 'newSession':
          hub.buildNewSession(message.data.shift(), message.transportName,
            function(err) {
              if (err) {
