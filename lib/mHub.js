      ...
      if (err) {
        ...
        for (var i = 0; i < items.length; i++) {
          if ('.js' == items[i].substr(items[i].length - 3).toLowerCase()) {
            ...
            //that.mH.addAdjunct(efact_name, temp_req);
            loaded++;
            
            // Add any loaded engines to the SessionGenerator...
            mSession.addEngine(temp_req);
          }
          else {
    ...
    taps: {
      ...
      "mEngine": {
        "SELF_DESCRIBE": function(me, msg, adjunctID){
          ...
          for (var adj in me.interface_spec.adjuncts) {
            if (adj.schema.type === 'mEngine') {
              //TODO: the following simplistic check makes version-control in engines 
              //        harder than it should be.
              engineConfig = adj.self_description;
              if (name === engineConfig.identity) {
                // Pass the factory method into the session so it can instance it.
                me.interface_spec.adjuncts[adjunctID].addEngine(adj.init, msg.data.args[1]);
              }
            }
          }
          return false; // don't emit
