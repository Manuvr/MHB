...
var argParser = function(jsonBuff) {
    ...
    if([] !== jsonBuff.raw && jsonBuff.raw.length !== 0){
        ...
        for(var i = 0; i < jsonBuff.raw.length; i++){
            ...
            // !!!! check the iteration on this
            sliceLen = i - iStart;

            outObj[argNum] =
            argRef[handler.argChoices[i - 1][i][argArrPos]].val(
...
// we'll fill this object with parsed stuff...
var dynamicCommand = {}
...
var argRef = {
    ...
    0x02    :   { len: 2, val: function(buff){return buff.readInt16LE()}}     ,
    0x03    :   { len: 4, val: function(buff){return buff.readInt32LE()}}      ,
    0x04    :   { len: 8, val: function(buff){return buff.readInt32LE()}}     , // THIS NEEDS TO CHANGE... bitshifting
    0x05    :   null    ,
