...
var runIt = function(jsonBuffer) {
    ...
    if(commands[jsonBuffer.messageId].def === "LEGEND_MESSAGES"){
            console.log("PARSING THE LEGEND... HOLD ON TO YO BUTTS");
...
var legendParser = function(jsonBuff){
    ...
    while(i < jsonBuff.raw.length) {
        ...
        while(doubleZero === 0){
            ...
            if(jsonBuff.raw.readUInt16LE(zeroLoc + 1) === 0x00) {
                doubleZero = 1;
            } else {
                while(startLoc < zeroLoc) {
                    if(argRef[jsonBuff.raw.readUInt8(startLoc)].len === 0){
                        isDynamic = 1;
                    }
    ...
    commands = tempObj;
    console.log("YOU WILL GO TO SPACE TODAY. COMMANDS INSERTED.");
}


var argParser = function(jsonBuff) {

    // this will need to pull from "staticCommand" and "dynamicCommands"
    // we'll first check the statics, then the dynamics in an if block
