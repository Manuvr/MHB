...
var execute_IMU_MAP_STATE = function(jsonBuff) {
    ...
    for (var b = 0; b < Object.keys(gm).length; b++) {
        ...
        for (f = 0; f < imuBaseProperties.length; f++) {
                ...
                // deal with error/temp not being included yet
                if (f < 9) {
                    if (set < 51) {
                    //console.log(imuBase + " - " + imuBaseReading + " - " + set + " - " + countSets + " - " + f);
                    gm[imuBase][imuBaseReading] = argObj[set][argObjSub[countSets]];
                    }
                    else {
                        console.log("Arg array is larger than can be accepted");
                    }
...
var commands = {
    ...
    58401:
    { flag: 0,
        argForms: { '616': [ 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18,
