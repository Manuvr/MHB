...
function mCore(transport) {
  ...
  this.sendSync = function() {
    if (that.syncCount < 25) {
      ...
      that.send('log', ['This used to be a blank emit. Interval cleared.', 7]);
    }
  }
  ...
  that.receiver.ee.on('outOfSync', function(outOfSync, reason) {
    if (outOfSync) {
      ...
      that.send('log', ['Became desync\'d because ' + reason + '.', 6]);
    } else {
      ...
      that.send('log', ['Became sync\'d and sent a KA.', 6]);
    }
  });
      ...
      inputs: {
        'syncForce': {
          ...
          func: function(me, data) {
            that.send('log', ['AM I SYNCD? ' + data, 5]);
      ...
      "mTransport" : {
        "data" : function(me, msg, adjunctID) {
          if (Buffer.isBuffer(msg.data)) {
            ...
            me.send('log', ['Transport got datas', 2]);
          }
          else {
            // Invalid
            me.send('log', ['Transport-originated data was not a buffer?? It was: '+ typeof data, 2]);
          }
