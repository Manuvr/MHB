  ...
  var fromEngine = function(method, data) {
    ...
    switch (method) {
      ...
      case 'sync':
        ...
        if (!that.interface_spec.members.engine.schema.state['sessionEstablished'].value) {
      ...
      case 'transport':
        ...
        break;
      case 'config':
        if (_has(that.interface_spec.members, 'engine')) {
          _defaultsDeep(that.interface_spec.members.engine, _cloneDeep(data));
        } else {
          that.interface_spec.members.engine = _cloneDeep(data);
        }
        break;
    ...
    if (_has(that.interface_spec.members, ['engine', 'schema.', 'outputs', method, 'state']) && data !== undefined) {
      that.interface_spec.members.engine.schema.state[that.interface_spec.members.engine.schema.outputs[method].state].value = data;
    }
    else {
  ...
  var fromTransport = function(method, data) {
    switch (method) {
      ...
      case 'connected':
        ...
        break;
      case 'config':
        if (_has(that.interface_spec.members, 'transport')) {
          _defaultsDeep(that.interface_spec.members.transport, _cloneDeep(data));
        } else {
          that.interface_spec.members.transport = _cloneDeep(data);
        }
        break;
      case 'log': // passthrough
      default:
    ...
    if (_has(that.interface_spec.members, ['transport', 'outputs', method, 'state']) && data !==
      undefined) {
      that.interface_spec.members.transport.state[that.interface_spec.members.transport.outputs[method].state].value =
        data;
    } else {
      // didn't find a state to update...
    }
      ...
      case 'session':
        switch (method) {
          case 'getLiveConfig':
            ...
            toClient('session', 'config', that.interface_spec);
            break;
          case 'assign':
          ...
          case 'connect':
            ...
            if (that.interface_spec.members.transport.state.connected.value !== (data[0]=='true')) {
