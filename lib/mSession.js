...
/** This is an array of transports that might experience a connection event.. */
var transports = [];

/** This is an array of sessions that are active. */
var sessions = [];
  ...
  var fromEngine = function(method, data) {
    ...
    switch (method) {
      case 'client':
        ...
        break;
      case 'sync':
        if (!that.config.engine.state['sessionEstablished'].value) {
    ...
    if (_has(that.config, ['engine', 'outputs', method, 'state']) && data !== undefined) {
      that.config.engine.state[that.config.engine.outputs[method].state].value = data;
    }
    else {
...
/**
 * Adds an "mTransport" to the transport array. These are the transports
 *   that are listening for connections.
 *
 * @param  {function} xport This requires an mTransport function
 */
mSession.prototype.addTransport = function(xport) {
  xport.on('connected', function(xport) {
    var nu_session = new session(xport);
    //nu_session
    sessions.push(nu_session);
  });
  transports.push(xport);
}
