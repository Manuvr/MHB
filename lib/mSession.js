  ...
  var fromEngine = function(method, data) {
    ...
    switch (method) {
      case 'client':
        ...
        break;
      case 'sync':
        if (!that.interface_spec.members.engine.state['sessionEstablished'].value) {
      ...
      case 'config':
        if (_has(that.interface_spec.members, 'engine')) {
          ...
          that.interface_spec.engine = _cloneDeep(data);
        }
        break;
    ...
    if (_has(that.interface_spec.members, ['engine', 'outputs', method, 'state']) && data !== undefined) {
      that.interface_spec.members.engine.state[that.interface_spec.members.engine.outputs[method].state].value = data;
    }
    else {
