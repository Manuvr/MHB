...
var msgHandler = function(_intSpec, _parentThis) {
  ...
  var adjunctReceive = function(key, msg) {
    ...
    if(Array.isArray(msg.target)) {
      ...
      if(msg.target[0] === '_adjunctUpdate') {
        self.intSpec.adjuncts[key].instance.removeListener('output', adjunctReceive.bind(self, key));
        self.addAdjunct(key, self.intSpec.adjuncts[key].instance);
      } else {
        // check for taps
        if(_has(self.intSpec, ["taps", self.intSpec.adjuncts[key].type, msg.target[msg.target.length - 1]])) {
          // checks the adjunct functions and if it returns false, it blocks the message from emitting.
          if(self.intSpec.tap[self.intSpec.adjuncts[key].type][msg.target[msg.target.length - 1]](parentThis, msg, key) === true) {
  ...
  this.sendToOutput = function(msgOrTarget, _data) {
    console.log('msgOrTarget: '+msgOrTarget);
