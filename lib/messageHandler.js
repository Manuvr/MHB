...
var msgHandler = function(_intSpec, _parentThis) {
  ...
  var adjunctReceive = function(name, msg) {
    ...
    if(Array.isArray(msg.target)) {
      ...
      if(msg.target[0] === '_adjunctUpdate') {
        ...
        // ADDED 02.23.16 -- DEAL WITH NAMED TAPS.  BREAKING CHANGE.
        // TODO: THIS IS GROSS.  REMOVE CODE DUPLICATION AFTER SPRINT.
        if(_has(self.intSpec, ["taps", "names", name, msg.target[0]])) {
          if(self.intSpec.taps.names[name][msg.target[0]](parentThis, msg, name) === true) {
            ...
            if(_get(self.intSpec.adjuncts[name], ["hide_msg"]) === true){
              // not sending because hide_msg was specified... this may be dangerous
            } else {
              self.sendToOutput(msg);
            }
          }
        } else if(_has(self.intSpec, ["taps", "types", self.intSpec.adjuncts[name].type, msg.target[0]])) {
          // checks the adjunct functions and if it returns false, it blocks the message from emitting.
          if(self.intSpec.taps.types[self.intSpec.adjuncts[name].type][msg.target[0]](parentThis, msg, name) === true) {
  ...
  this.sendToOutput = function(msgOrTarget, _data) {
    ...
    _set(self.intSpec, ["schema", "outputs", outMsg.target[0], "value"], outMsg.data);
  ...
  this.removeMetaAdj = function(name, targetAdj) {
    delete self.intSpec.adjuncts[targetAdj]._alias
    delete self.intSpec.adjuncts[name].meta
    delete self.intSpec.adjuncts[name].instance
    delete self.intSpec.adjuncts[name]
    parentThis.send('_adjunctUpdate', {})
  }
  ...
  this.hideAdjunct = function(name, value, hide_msg) {
    self.intSpec.adjuncts[name].instance.intSpec.hidden = value;
    self.intSpec.adjuncts[name].hide_msg = hide_msg;
    parentThis.send('_adjunctUpdate', {})
  }
