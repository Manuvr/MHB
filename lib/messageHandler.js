...
var sampleAdjuncts = {
  "transport": {
    "obj": new Transport(),
    "actions": {
      "data": function(me, msg) {
        // do something
        me.doSomething()
        return false; // if returns false, it will NOT emit out of self...
      }
    }
  },
  "engine": {
    ...
    "actions": {
      "log": function(me, msg) {
        me.doSomething(msg.data);
        // you can mutate the msg, and it will be emitted back out altered
        msg.data.derp = "10"
        return true; // return true if you want it emitted back out
      }
    }
  }
};
...
var msgHandler = function(_intSpec, _adjuncts, _actions, _parentThis) {
  ...
  var adjunctReceive = function(key, msg) {
    ...
    if(Array.isArray(msg.target)) {
      ...
      if(_has(self.adjuncts, [key, "actions", msg.target[msg.target.length - 1]])) {
        // checks the adjunct functions and if it returns false, it blocks the message from emitting.
        if(self.adjuncts[key].actions[msg.target[msg.target.length - 1]](parentThis, msg) === true) {
