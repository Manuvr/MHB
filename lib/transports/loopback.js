  ...
  this.toTransport = function(method, data) {
    switch (method) {
      case 'connect':
        ...
        that.send('log', {
          body:      'Transport received a connection.',
          verbosity: 6
        });
        break;
      case 'data':
        that.send('log', {
          body:      'Transport received data: '+data.toString(),
          verbosity: 7
        });
      default:
  ...
  this.getWorker = function() {
    ...
    nu_loop.once('lb_spawn_complement',
      function() {
        ...
        cp_loop.interface_spec.schema.output.remoteAddress.value = nu_loop.interface_spec.schema.output.localAddress.value;
        nu_loop.interface_spec.schema.output.remoteAddress.value = cp_addr;
