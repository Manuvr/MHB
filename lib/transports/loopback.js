  ...
  var toTransport = function(method, data) {
    switch (method) {
      ...
      case 'data':
        ...
        break;
      case 'config':
        that.emit('fromTransport', 'config', interface_spec.members.transport);
        break;
      case 'scan':
        that.emit('toDevice', method, data);
        break;
...
function mTransportFactory() {
  ...
  var emitOutput = function(method, args) {
    switch (method) {
      case 'connected':
        that.emit('emitOutput', 'connected', args);
        break;
      case 'log':
      default:
        that.emit('emitOutput', method, args);
        break;
    }
  }
    ...
    function(message) {
      switch (message.method) {
        case 'connect':
          ...
          emitOutput('connected', instance_of_transport);
          break;
        case 'config':
          emitOutput('config', interface_spec);
          break;
        case 'scan':
